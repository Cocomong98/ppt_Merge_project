...
try:
    ...
    # Mac 개발 환경이거나 pywin32가 설치되지 않은 경우
    WINDOWS_ENV = False 
        ...
        try:
            ...
            for path in self.file_paths:
                if path.lower().endswith('.ppt'):
                    # win32com을 사용하려면 Windows 환경이어야 함을 다시 확인
                    if not WINDOWS_ENV:
                         raise EnvironmentError("이 PC는 Windows 환경이 아닙니다. `.ppt` 변환은 Windows에서만 가능합니다.")
...
class PptxMergerApp(QWidget):
    def __init__(self):
        super().__init__()
        # win32com 사용 여부에 따라 타이틀 명시
        title_suffix = " (MS PowerPoint 제어)" if WINDOWS_ENV else " (pywin32 설치 필요)"
        self.setWindowTitle('PPTX/PPT 순서 병합 프로그램' + title_suffix) 
    ...
    def setup_ui(self):
        ...
        if WINDOWS_ENV:
            drag_info += "\n(⚠️ .PPT 변환: MS PowerPoint 설치 및 관리자 권한 필수)"
    ...
    def execute_merge(self):
        ...
        self.status_label.setText("상태: 병합 작업 시작 중... (PPT 파일 변환 포함)")
    ...
    def on_progress_update(self, current, total):
        ...
        # 변환 단계가 있을 경우 ProgressDialog 표시 변경
        if total == 1 and current == 1:
            self.progress_dialog.setLabelText("PPT 파일을 PPTX로 변환 중... (MS PowerPoint 제어)")
